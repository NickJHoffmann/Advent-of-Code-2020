...
def findTrees(inputData, xSlope, ySlope):
    lineLength = len(inputData[0])   # Max length of each line, stored for readability later
    treeCount = 0
    xPos = 0              # Current horizontal position
    currentLine = 0
    while currentLine < len(inputData):
        row = inputData[currentLine].strip()
        if row[xPos] == '#':
            treeCount += 1
        xPos += xSlope
...
# Load data to reduce I/O time
with open(filename, 'r') as file:
    mapRows = file.readlines()

# Part 1
part1 = findTrees(mapRows, 3, 1)
...
for xSlope, ySlope in slopes:
    numOfTrees *= findTrees(mapRows, xSlope, ySlope)
